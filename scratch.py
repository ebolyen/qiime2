...
def core_metrics(ctx, table, phylogeny, counts_per_sample):
    ...
    r = bray_curtis_distance_matrix.block_until_ready()
...
handler = Foo(a=1, b=2, c=3)


[EXECUTION CONTEXT]
handler: SlurmHandler
...
Partial @ (SemanticType % Foo)

Partial[SemanticType % Foo]
Partial(SemanticType % Foo)


List[SemanticType % Foo]
Partial[SemanticType % Foo]
Optional[SemanticType % Foo]

Optional[List[Partial[T]]] -> T

Optional(List(Partial(T)))

Partial(SemanticType % Property('foo'))


SemanticType % Property('Foo')
