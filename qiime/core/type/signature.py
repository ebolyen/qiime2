...
class ParameterSpec(ImmutableBase):
    ...
    def __eq__(self, other):
        if self.has_qiime_type() != other.has_qiime_type():
            return False
        if not self.qiime_type.equals(other.qiime_type)):
            return False

        if self.has_view_type() != other.has_view_type():
            return False
        if self.view_type is not other.view_type:
            return False
...
class PipelineSignature:
    ...
    def _parse_signature(self, callable, inputs, parameters, outputs):
        ...
        for name, parameter in inspect.signature(callable).parameters:
            if parameter.kind.startswith('VAR_'):
